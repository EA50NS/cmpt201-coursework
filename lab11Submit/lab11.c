...
int main() {
  ...
  // TODO: CHECK ERROR FOR FUNCTION CALL
  FILE *pub_key_FD = fopen("public_key.pem", "r");
  EVP_PKEY *pubkey = PEM_read_PUBKEY(pub_key_FD, NULL, NULL, NULL);
...
#define MAX_FILE_SIZE 512
  ...
  memcpy(message, &message_path, sizeof(message_path));
  memcpy(signature, &sign_path, sizeof(sign_path));

  EVP_MD_CTX *ctx = EVP_MD_CTX_new();
  EVP_DigestVerifyInit(ctx, NULL, EVP_sha256(), NULL, pubkey);
  EVP_DigestVerifyUpdate(ctx, message, sizeof(message));
  if (1 == EVP_DigestVerifyFinal(ctx, signature, sizeof(signature))) {
    return 1;
  } else {
    return -1;
  }
