...
2. What data is the client sending to the server?
  Clients sends an array of strings.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  run_acceptor accepts a struct of acceptor arguments which contains three
things: whether or not the thread is running, the handle of the list, and the
list mutex
...
3. What does `main()` do with the received messages?
  It waits for enough messages to be sent and creates threads for the messages
to be processed

4. How are threads used in this sample code?
  Threads are used for each message for processing.
*/
